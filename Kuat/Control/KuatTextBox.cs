	...
	public class KuatTextBox : KuatControl
	{
		...
		protected override void OnPaint(object sender, NvgContext e)
		{
			...
			else
			{
				...
				e.BeginPath();
				e.Rect(ClientLocation.X + 2 + w, ClientRectangle.Y + 2, selectedWidth, Size.Height - 4);
				e.Fill();
		...
		protected virtual bool IsValidCharacter(char c)
		{
			return true;
		}
		...
		protected override void OnKeyDown(object sender, KeyboardKeyEventArgs e)
		{
			...
			switch (e.Key)
			{
				...
				case Key.Left:
					if (e.Control)
					{
						var originalPos = SelectionStart;

						var firstTest = true;
						for (var i = SelectionStart; i >= 1; i--)
						{
						...
						if (SelectionStart == originalPos)
							SelectionStart = 0;

						if (e.Shift)
							SelectionLength += originalPos - SelectionStart;
						else
							SelectionLength = 0;
					}
					else
				...
				case Key.Right:
					if (e.Control)
					{
						...
						var firstTest = true;
						for (var i = SelectionStart; i < Text.Length - 1; i++)
						{
							if (char.IsLetterOrDigit(Text[i + 1]))
							{
								firstTest = false;
